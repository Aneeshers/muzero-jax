#!/bin/bash
#SBATCH --job-name=cartpole_muax                   # Default job name (sweep overrides)
...
#SBATCH -o slurm-%x-%j_cartpole_muax.out                 # STDOUT (%x=jobname, %j=jobid)
#SBATCH -e slurm-%x-%j_cartpole_muax.err                 # STDERR
...
cd /n/home04/amuppidi/muax_control/muax_simple
# Use explicit Python path from your torch env (as in your example)
~/.conda/envs/torch/bin/python train_cartpole.py
