#!/bin/bash
#SBATCH --job-name=jax_cartpole_muax                   # Default job name (sweep overrides)
...
#SBATCH -o slurm-%x-%j_jax_cartpole_muax.out                 # STDOUT (%x=jobname, %j=jobid)
#SBATCH -e slurm-%x-%j_jax_cartpole_muax.err                 # STDERR
