...
def fit(model, 
  ...
  if model_save_path is None:
    ...
    # NEW: carry params / opt_state explicitly
    params = model.params
...
#         r = 1 / (1 - tracer.gamma)
    ...
    for _ in range(num_update_per_episode):
      transition_batch = buffer.sample(
          num_trajectory=num_trajectory,
          sample_per_trajectory=sample_per_trajectory,
          k_steps=k_steps,
      )

      # Pure-style update: explicit params / opt_state
    ...
    if ep % save_every_n_epochs == 0:
      ...
      cur_path = os.path.join(model_dir, model_folder_name, save_name) 
            # Sync model instance with latest params/opt_state before saving
    ...
    if ep % test_interval == 0:
      # Sync model instance with latest params/opt_state before testing
      model._params = params
      model._opt_state = opt_state
